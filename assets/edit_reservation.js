...
let conform = true;
let detailsReservationCalled = false;
...
function updateTimeSlots() {
  ...
  fetch("/api?date=" + dateInput.value)
    ...
    .then(function (data) {
      ...
      if (
        (data.maxReservationLunch <= 0 || data.maxSeatLunch <= 0) &&
        (data.maxReservationDiner <= 0 || data.maxSeatDiner <= 0)
      ) {
      ...
      if (!detailsReservationCalled) {
        setTimeReservation(data);
        detailsReservationCalled = true;
      }
...
function setTimeReservation(data) {
  if (data.hour) {
...
reservationForm.addEventListener("submit", function (event) {
  ...
  let selectedOption =
    howManyGuestSelector.options[howManyGuestSelector.selectedIndex];
